...
def preprocess_signature(img, canvas_size, img_size=(170, 242), input_size=(150, 220)):
    ...
    h,w = canvas_size
    resized = cv2.resize(inverted, (w,h), interpolation=cv2.INTER_LINEAR)
...
def normalize_image(img, size=(840, 1360)):
    ...
    desire_width = int(img_r * desire_ratio)
    desire_heigh = int(img_c / desire_ratio)

    if desire_heigh > img_r:
        start_h = (desire_heigh - img_r) // 2
        new_img = np.ones((desire_heigh, img_c), dtype=np.uint8) * 255
        new_img[start_h:start_h + img_r, 0: img_c] = cropped
    else:
        start_w = (desire_width - img_c) // 2
        new_img = np.ones((img_r, desire_width), dtype=np.uint8) * 255
        new_img[0:img_r, start_w: start_w + img_c] = cropped
